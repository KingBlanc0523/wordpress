...
function st_custom_admin_notice() {
	...
	<div class="notice notice-success is-dismissible st_notice" data-dismissible="disable-done-notice-forever">
		<div>
			<p>	
			<?php _e('Thank you for using the free version of ','styled-store'); ?>
			<?php echo esc_html( $st_th_info->get('Name') );?> - 
			<?php echo esc_html( $st_th_info->get('Version') );
			 ?>
			<?php _e('theme. Want more features? Check out the', 'styled-store'); ?>
			<a href="<?php echo esc_url( 'https://www.styledthemes.com/themes/styled-store-pro/?utm_source=FreeThemes&utm_medium=UpdateMsg&utm_campaign=StyledStore');?>" target="_blank" aria-label="Dismiss the welcome panel">
				<strong><?php _e('PRO version','styled-store');?></strong>
			</a>
			<?php _e('for more options and professional support!', 'styled-store'); ?>
