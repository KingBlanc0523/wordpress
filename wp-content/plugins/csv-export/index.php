            ...
            while ( $query->have_posts() ) {
                ...
                if ( $order->details->post_status == $order_status || $order_status == 'any' ) {
                    ...
                    if ( isset( $_POST['col_payment_date'] ) ) {
                        ...
                        $payment_date = tc_format_date( strtotime( $order->details->post_date_gmt ) );
                        $tc_payment_array = array(
                            __( 'Payment Date', 'tccsv' ) => $payment_date,
                        );
                    } else {
                        $tc_payment_array = array();
                    }
                    ...
                    if ( isset( $_POST['col_order_total'] ) ) {
                        ...
                        $tc_order_total_array = array(
                            __( 'Order Total', 'tccsv' ) => round( $order_total, 2 ),
                        );
                    } else {
                        $tc_order_total_array = array();
                    }
                    ...
                    if ( isset( $_POST['col_order_total_once'] ) ) {
                        ...
                        if ( $i > 1 ) {
                            ...
                            if ( $tc_post_type == 'shop_order' ) {
                                $order = wc_get_order( $order->id );
                                $order_total_once = $order->get_total();
                            } else {
