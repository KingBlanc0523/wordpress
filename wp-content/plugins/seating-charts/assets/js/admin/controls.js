...
//$(".tc-zoom-slider").slider("option", window.tc_seat_zoom_level)
        ...
        save_confirmation: function () {
            ...
            //kingblanc editor
            var ticket_type_name = $("#ticket_type_id option:selected").html();
            console.log("======1=======>"+ticket_type_name);
            if('Temporarily Unavailable Seats' == ticket_type_name){
                tc_controls.save_confirmation_callback(true);
                return false;
            }
            console.log("======2=======>"+ticket_type_name);
...
// Get inset property
        ...
        get_event_ticket_types: function () {
            ...
            $.post(tc_controls_vars.ajaxUrl, {action: "tc_get_event_ticket_types", event_id: event_id}, function (data) {
                ...
                $('select[name=ticket_type_id]').on('change', function () {
                    var inner__html = '';
                    if ($('select[name=ticket_type_id] option:selected').html() == 'Send Tickets to User') {
                        inner__html = $('#input_user').html();
                    } else {
                        inner__html = $('#common').html();
                    }
                    $('#tc-seat-labels-settings').html(inner__html);
                    if ($('.tc_set_seat ui-selected').length > 1) {
