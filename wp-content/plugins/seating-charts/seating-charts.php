...
if ( !class_exists( 'TC_Seat_Chart' ) ) {
    class TC_Seat_Chart
    {
        ...
        function __construct()
        {
            ...
            add_action('wp_ajax_tc_get_user_id',array($this,'get_user_id'));
        ...
        function get_user_id()
        {
            ...
            if (!$user_name) {
                echo json_encode(array('status' => 0, 'user_id' => 0));
            } else {
                $queryResult = $wpdb->get_col("select ID from w_users where user_login = '{$user_name}' or user_email = '{$user_name}'");
                if (count($queryResult)) {
                    echo json_encode(array('status' => 1, 'user_id' => $queryResult[0]['ID']));
                } else {
                    echo json_encode(array('status' => 0, 'user_id' => $wpdb -> last_query));
                }
            }
            exit;
        }
